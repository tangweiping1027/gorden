<template>
  <div class="contain" ref="dragContain">
    <div class="control" v-drag="contain">
      <div style="line-height:30px;background:#409EFF;cursor:move;color:#fff;text-indent:15px;">控制面板</div>
      ...
      if (type) {
        ...
        document.onmousemove = function(e) {
          ...
          that.json[that.divindex].style.width =
            (newX + parseInt(that.movejson.moveWidth) >= 80
              ? newX + parseInt(that.movejson.moveWidth)
              : '80') + 'px'
          that.json[that.divindex].style.height =
            (newY + parseInt(that.movejson.moveHeight) >= 50
              ? newY + parseInt(that.movejson.moveHeight)
              : '50') + 'px'
        }
